...
static bool tud_audio_clock_set_request(
	uint8_t rhport,
...
static bool tud_audio_feature_unit_get_request(
	...
	if (
		...
		if (request->bRequest == AUDIO_CS_REQ_RANGE) {
			...
			TU_LOG1(
				"Get channel %u volume range (%d, %d, %u) dB\r\n",
...
static bool tud_audio_feature_unit_set_request(
    uint8_t rhport, audio_control_request_t const* request,
    uint8_t const* buf
) {
	(void)rhport;
...
bool tud_audio_set_itf_close_EP_cb(
	...
	if (ITF_NUM_AUDIO_STREAMING == itf && alt == 0) blink_interval_ms = BLINK_MOUNTED;
	return true;
}
...
bool tud_audio_set_itf_cb(
	...
	if (ITF_NUM_AUDIO_STREAMING == itf && alt != 0) blink_interval_ms = BLINK_STREAMING;
	#if CFG_AUDIO_DEBUG
...
void tud_audio_feedback_params_cb(
	uint8_t func_id, uint8_t alt_itf,
...
#if CFG_AUDIO_DEBUG
	bool tud_audio_rx_done_post_read_cb(
		uint8_t rhport, uint16_t n_bytes_received,
...
void audio_task(void) {
	...
	if (current_sample_rate == 44100 && (curr_ms % 10 == 0)) {
		// Take one more sample every 10 cycles, to have a average reading speed
		// of 44.1 This correction is not needed in real world cases
		...
		// Take one more sample every 5 cycles, to have a average reading speed
		// of 88.2 This correction is not needed in real world cases
		length += CFG_TUD_AUDIO_FUNC_1_N_BYTES_PER_SAMPLE_RX * CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX;
	}
...
void led_blinking_task(void) {
	...
	void audio_debug_task(void) {
		...
		if (tud_hid_ready()) tud_hid_report(0, &debug_info, sizeof(debug_info));
	}
