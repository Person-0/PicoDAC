...
bool tud_audio_rx_done_post_read_cb(uint8_t rhport, uint16_t n_bytes_received,
  ...
  fifo_count_avg = (uint32_t)(((uint64_t)fifo_count_avg * 63 +
                               ((uint32_t)fifo_count << 16)) >>
                              6);
...
void audio_task(void) {
  ...
  uint16_t length = current_sample_rate / 1000 *
  ...
  if (current_sample_rate == 44100 && (curr_ms % 10 == 0)) {
    ...
    length += CFG_TUD_AUDIO_FUNC_1_N_BYTES_PER_SAMPLE_RX *
              CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX;
